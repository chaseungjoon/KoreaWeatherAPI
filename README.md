...
| 필드명              | 의미                  | 값 (예시)                                | 해석                                |
|----------------------|-----------------------|------------------------------------------|-------------------------------------|
| frfrInfoId           | 산불 고유 ID          | 398504                                   | 이 건을 구분하는 고유 번호           |
| frfrLctnXcrd         | 산불 위치 경도        | 127.67243000000057                       | 산불 지점의 경도                     |
| frfrLctnYcrd         | 산불 위치 위도        | 37.08104000024055                        | 산불 지점의 위도                     |
| frfrSttmnLctnXcrd    | 최초 신고 경도        | 127.67243000000029                       | 최초 신고 위치(추정) 경도            |
| frfrSttmnLctnYcrd    | 최초 신고 위도        | 37.08104000012027                        | 최초 신고 위치(추정) 위도            |
| frfrSttmnAddr        | 산불 발생지 주소      | 충청북도 음성군 감곡면 영산리            | 행정구 단위 주소                     |
| frfrSttmnAddrDe      | 산불 발생지 상세주소  | 충청북도 음성군 감곡면 영산리 산55-3임   | 세부 위치 (임야 표시 포함)           |
| frfrPrgrsStcd        | 산불 진행 상태 코드   | 02                                       | 코드값 (02 → “진화중”)               |
| frfrPrgrsStcdNm      | 산불 진행 상태 이름   | 진화중                                   | 현재 진화 중 단계                    |
| frfrOccrrTpcd        | 발생 원인 구분 코드   | 05                                       | 원인 분류 코드 (공식 매핑표 필요)    |
| frfrOccrrStcd        | 발생 상태 코드        | 31                                       | 상태 코드 (의심, 확정 등 구분)        |
| frfrStepIssuCd       | 대응 단계 코드        | 00                                       | 대응단계 코드값                      |
| frfrStepIssuNm       | 대응 단계 이름        | 초기 대응                                | 현재는 초기 대응 단계                |
| frfrSttmnDt          | 신고 일자             | 20250822                                 | YYYYMMDD (2025년 8월 22일)           |
| frfrSttmnHms         | 신고 시각             | 144122                                   | HHMMSS (14:41:22)                    |
| frfrFrngDtm          | 발생(신고) 일시       | 2025-08-22 14:41:22                      | 사람이 읽기 쉽게 표현된 시각         |
| lgdngCd              | 행정동 코드           | 4377037026                               | 법정동 코드 (행정구역 고유번호)      |
---
